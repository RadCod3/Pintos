...
start_process(void *file_name_) {
    ...
    /*Code to get seperate strings from string file_name*/
    char *tokenizer_ptr;
    int argCount = 0;

    for (char *token = strtok_r(file_name, " ", &tokenizer_ptr); token != NULL; token = strtok_r(NULL, " ", &tokenizer_ptr)) {
        args[argCount] = token;
        argCount++;
    }
...
void args_to_stack(int argCount, char **line, void **esp) {
    ...
    // storing arguments in stack
    for (int i = argCount - 1; i >= 0; i--) {
        // decrement stack ptr to allocate memory for the size of the arg
        *esp -= strlen(line[i]) + 1;

        // store address of arg in the stack in the list of arg addresses
        args[i] = *esp;
